...
| `Credit_WhenAmountIsLessThanZero_ShouldThrowArgumentOutOfRange` | Проверяет выброс исключения при отрицательной сумме кредита |
...
После замены `m_balance += amount` на `m_balance -= amount` в методе `Debit` все 5 тестов успешно прошли.
